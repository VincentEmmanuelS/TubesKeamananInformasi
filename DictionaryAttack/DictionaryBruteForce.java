 ...
 */
 ...
 public class DictionaryBruteForce implements DictionaryAttack {
     ...
     private String processWord(String word, String hashedPassword, PasswordHasher passwordHasher) throws NoSuchAlgorithmException {
         ...
         // Check mutations
         for (int i = 1; i <= 3; i++) {
             String mutation = checkMutations(word, i, true, hashedPassword, passwordHasher);
             if (mutation != null) {
                 return mutation;
             }
 
             mutation = checkMutations(word, i, false, hashedPassword, passwordHasher);
             if (mutation != null) {
                 return mutation;
             }
         }
         ...
         for (int i = 1; i <= 2; i++) {
             for (int j = 1; j <= 2; j++) {
                 String mutation = checkFrontAndBackMutations(word, i, j, hashedPassword, passwordHasher);
                 if (mutation != null) {
                     return mutation;
                 }
             }
     ...
     private String checkMutations(String base, int count, boolean addToFront, String hashedPassword, PasswordHasher passwordHasher) throws NoSuchAlgorithmException {
         return generateCombinationsAndCheck(new StringBuilder(), count, base, addToFront, hashedPassword, passwordHasher);
     }
     ...
     private String generateCombinationsAndCheck(StringBuilder current, int remaining, String base, boolean addToFront, String hashedPassword, PasswordHasher passwordHasher) throws NoSuchAlgorithmException {
         if (remaining == 0) {
         ...
         for (char c : CHARSET) {
             current.append(c);
             String result = generateCombinationsAndCheck(current, remaining - 1, base, addToFront, hashedPassword, passwordHasher);
             if (result != null) {
                 return result;
             }
